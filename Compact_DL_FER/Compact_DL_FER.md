...
为了避免受试者同时出现在训练和测试集中，我们按照ID升序将其分为10个子集，这与10折交叉验证方案相同。该实验方案用于本文实验比较中包含的所有方法。   表1显示了CK +数据库上10折交叉验证的总体准确性。我们基于框架的方法的准确性优于大多数基于序列的方法，仅次于峰引导方法。然而，在[47]中，它们使用CK+ 数据库一千倍大小的500K张图像预训练CNN模型。结果表明提出的CNN模型非常适合在小的数据库上学习面部表情。![表1]()  
在Oulu-CASIA数据库上的表情识别更具挑战性，因为它包含了更多低亮度表情，很难区分低分辨率图像。然而，Oulu-CASIA数据库仍然是好的基准，每个类别有着完整的表情样本。我们的方法的结果优于在Oulu-CASIA数据库上的所有其他方法，如表2所示。![表2]()
