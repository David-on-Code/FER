...
尽管基于基本情感的情感模型在表达我们日常情感展示的复杂性和微妙性的能力方面受到限制，并且其他情绪描述模型，例如面部动作编码系统（FACS）和the continuous model using affect dimensions 被认为代表更广泛的情感，由于其开创性的研究以及对面部表情的直接直观定义，用离散基本情感描述情感的分类模型仍然是FER最受欢迎的观点。    
...
##### 4.1.8 Discussion  
现有的结构良好的深度FER系统着重于两个关键问题：缺乏足够的多样化训练数据以及与表情无关的变化，例如照明，头部姿势和身份。 表6显示了这些不同类型的方法相对于两个未解决的问题（数据大小要求和与表情无关的变化）和其他重点（计算效率，性能和网络训练难度）的相对优缺点。  
预训练和微调已成为FER领域的主流技术，以解决训练数据不足和过拟合的问题。被证明特别有用的实用技术是使用辅助数据，从大规模目标或面部识别数据集到小型FER数据集，即从大到小，从一般到特殊，对网络进行多阶段预训练和微调。但是，与端到端训练框架相比，与表达情无关的表示结构仍保留在现成的预训练模型中，例如 the large domain gap with the objection net and the subject identification distraction in the face net。因此，提取的特征通常容易受到身份变化的影响，并且性能会下降。值得注意的是，随着大规模的in the wild FER数据集（例如AffectNet和RAF-DB）的出现，使用深层的端到端训练中等规模的网络也可以取得竞争优势。   
除了直接使用原始图像数据来训练深层网络，使用diverse pre-designed features以增强网络对常见干扰（例如照明，头部姿势和遮挡）的鲁棒性，并迫使网络将更多注意力集中在具有表情信息的面部区域。此外，使用多个异构输入数据可以间接扩大数据大小。但是，这种方法通常忽略了身份偏差问题。此外，生成多样化的数据会占用更多时间，并且将这些多个数据组合会导致高维度，这可能会影响网络的计算效率。  
训练具有大量隐藏层和灵活过滤器的深度和广域网络是学习区分目标任务的深层高级特征的有效方法。但是，此过程容易受到训练数据量的影响，如果训练不足，则会表现不佳。数据可用于学习新参数。并行或串联集成多个相对较小的网络是克服此问题的自然研究方向。Network ensemble在特征或决策级别集成各种网络以结合其优势，通常在情感竞赛中应用以帮助提高性能。然而，设计不同种类的网络来相互补偿会明显增加计算成本和存储需求。此外，通常根据原始训练数据的性能来学习每个子网的权重，从而导致对新的，未经测试的数据进行过度拟合。Multitask  networks考虑到目标FER任务与其他次要任务（例如面部界标定位，面部AU识别和面部验证）之间的交互作用，多任务网络共同训练了多个网络，因此可以很好地消除与表情的因素，包括身份偏差。这种方法的缺点是，它要求所有任务都带有标记数据，并且随着涉及更多任务，训练变得越来越麻烦。可替代地，cascaded networks以分级方法顺序地训练多个网络，在这种情况下，所学习特征的辨别能力不断增强。通常，此方法可以缓解过度拟合的问题，同时可以逐步消除与面部表情无关的因素。值得考虑的缺陷是，大多数现有的级联系统中的子网都是在没有反馈的情况下进行单独训练，而端到端的训练策略对于提高训练效果和性能更为可取。  
理想情况下，深度网络（尤其是CNN）具有处理头部变化的良好功能，但是大多数当前的FER网络并未明确解决头部变化，并且未在自然场景中进行测试。可以利用生成对抗网络（GANs）来解决此问题，方法是正面化人脸图像，同时保留表情特征或合成任意姿势以帮助训练姿势不变网络。GAN的另一个优点是，可以通过生成相应的中性人脸图像或合成不同的表情来明确区分身份变化，同时保留身份不变FER的身份信息。 此外，GAN可以帮助增加规模和多样性方面的培训数据。 GAN的主要缺点是训练不稳定以及视觉质量和图像多样性之间的权衡。
